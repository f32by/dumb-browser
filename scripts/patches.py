...
def apply_patches(patch_path_iter, tree_path, reverse=False, patch_bin_path=None, dry_run=False, reverse_patch=False):
    """
...
def _dry_callback(args):
    logger = get_logger()
    for patch_dir in args.patches:
        logger.info('Dry-run apply patches from %s', patch_dir)
        apply_patches(
            generate_patches_from_series(patch_dir, resolve=True),
            args.target,
            patch_bin_path=args.patch_bin,
...
def _reverse_callback(args):
    logger = get_logger()
    for patch_dir in args.patches:
        logger.info('Reverse patches from %s', patch_dir)
        apply_patches(
            generate_patches_from_series(patch_dir, resolve=True),
            args.target,
            patch_bin_path=args.patch_bin,
...
def main():
    ...
    dry_parser = subparsers.add_parser(
        'dry', help='Applies patches (in GNU Quilt format) to the specified source tree')
    dry_parser.add_argument(
        '--patch-bin', help='The GNU patch command to use. Omit to find it automatically.')
    dry_parser.add_argument('target', type=Path, help='The directory tree to apply patches onto.')
    dry_parser.add_argument(
        'patches',
        type=Path,
        nargs='+',
        help='The directories containing patches to apply. They must be in GNU quilt format')
    dry_parser.set_defaults(callback=_dry_callback)

    reverse_parser = subparsers.add_parser(
        'reverse', help='Applies patches (in GNU Quilt format) to the specified source tree')
    reverse_parser.add_argument(
        '--patch-bin', help='The GNU patch command to use. Omit to find it automatically.')
    reverse_parser.add_argument('target', type=Path, help='The directory tree to apply patches onto.')
    reverse_parser.add_argument(
        'patches',
        type=Path,
        nargs='+',
        help='The directories containing patches to apply. They must be in GNU quilt format')
    reverse_parser.set_defaults(callback=_reverse_callback)
