...
static std::vector<MatcherRule> kMatcherRules = {
  // Global
  {
    ".*",
...
private:
  void AddMatchers() {
    for(const auto& rule: kMatcherRules) {
...
int ApplyQueryFilters(const GURL& request_url, std::string& new_query) {
  int count = 0;
  const std::string& full_url = request_url.spec();

  return GetMatcher().FilterQuery(full_url, new_query);
}
