...
#include "dumb_browser/browser/net/dumb_proxying_web_socket.h"
...
// #include "dumb_browser/common/network_constants.h"
#include "net/cookies/site_for_cookies.h"

// TODO: fix this
const char kEmptyDataURI[] = "data:text/plain,";

DumbProxyingWebSocket::DumbProxyingWebSocket(
...
// static
DumbProxyingWebSocket* DumbProxyingWebSocket::ProxyWebSocket(
...
void DumbProxyingWebSocket::Start() {
  ...
  if (proxy_has_extra_headers()) {
    continuation = base::BindRepeating(
        &DumbProxyingWebSocket::ContinueToStartRequest,
        weak_factory_.GetWeakPtr());
  } else {
    continuation = base::BindRepeating(
        &DumbProxyingWebSocket::OnBeforeRequestComplete,
        weak_factory_.GetWeakPtr());
  }
...
DumbProxyingWebSocket::web_socket_factory() {
  return base::BindOnce(&DumbProxyingWebSocket::WebSocketFactoryRun,
                        base::Unretained(this));
}

mojo::Remote<network::mojom::WebSocketHandshakeClient>
DumbProxyingWebSocket::handshake_client() {
  return std::move(forwarding_handshake_client_);
}

bool DumbProxyingWebSocket::proxy_has_extra_headers() {
  return proxy_trusted_header_client_.is_bound();
}

void DumbProxyingWebSocket::WebSocketFactoryRun(
    const GURL& url,
...
void DumbProxyingWebSocket::OnOpeningHandshakeStarted(
...
void DumbProxyingWebSocket::ContinueToHeadersReceived() {
...
void DumbProxyingWebSocket::OnConnectionEstablished(
...
void DumbProxyingWebSocket::OnAuthRequired(
...
void DumbProxyingWebSocket::OnBeforeSendHeaders(
...
void DumbProxyingWebSocket::OnHeadersReceived(
...
void DumbProxyingWebSocket::OnBeforeRequestComplete(int error_code) {
  ...
  if (proxy_has_extra_headers()) {
    proxy_trusted_header_client_->OnBeforeSendHeaders(
        request_.headers,
        base::BindOnce(
            &DumbProxyingWebSocket::OnBeforeSendHeadersCompleteFromProxy,
            weak_factory_.GetWeakPtr()));
  } else {
...
void DumbProxyingWebSocket::OnBeforeSendHeadersCompleteFromProxy(
    int error_code,
...
void DumbProxyingWebSocket::OnBeforeSendHeadersComplete(int error_code) {
  DCHECK(proxy_has_extra_headers() ||
         !receiver_as_handshake_client_.is_bound());
...
void DumbProxyingWebSocket::ContinueToStartRequest(int error_code) {
  if (error_code != net::OK) {
    OnError(error_code);
    return;
  }
  ...
  receiver_as_handshake_client_.set_disconnect_with_reason_handler(
      base::BindOnce(&DumbProxyingWebSocket::OnMojoConnectionError,
                     base::Unretained(this)));
}

void DumbProxyingWebSocket::OnHeadersReceivedCompleteFromProxy(
    int error_code,
...
void DumbProxyingWebSocket::OnHeadersReceivedComplete(int error_code) {
  if (error_code != net::OK) {
    OnError(error_code);
    return;
  }
  ...
  if (proxy_has_extra_headers()) {
    proxy_trusted_header_client_->OnHeadersReceived(
        ...
        remote_endpoint_,
        base::BindOnce(
            &DumbProxyingWebSocket::OnHeadersReceivedCompleteFromProxy,
            weak_factory_.GetWeakPtr()));
  } else {
...
void DumbProxyingWebSocket::PauseIncomingMethodCallProcessing() {
  receiver_as_handshake_client_.Pause();
  if (proxy_has_extra_headers())
    receiver_as_header_client_.Pause();
}

void DumbProxyingWebSocket::ResumeIncomingMethodCallProcessing() {
  receiver_as_handshake_client_.Resume();
  if (proxy_has_extra_headers())
    receiver_as_header_client_.Resume();
}

void DumbProxyingWebSocket::OnError(int error_code) {
  if (!is_done_) {
    is_done_ = true;
  }
...
// ResetWithReason
void DumbProxyingWebSocket::OnMojoConnectionError(
